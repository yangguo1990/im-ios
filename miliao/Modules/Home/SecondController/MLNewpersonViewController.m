...
@interface MLNewpersonViewController ()<UICollectionViewDelegate,UICollectionViewDataSource>
...
@implementation MLNewpersonViewController
...
-(void)giveML_newpersonApi{
    ...
    kSelf;
    ML_getTypeHostsApi *api = [[ML_getTypeHostsApi alloc]initWithtoken:token type:@"2" page:@(self.ML_Page) limit:@"20" location:@"" extra:[self jsonStringForDictionary]];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([collectionView isEqual:self.ML_homeCollectionView]) {
        if ([self.ML_newpersonArray[indexPath.item][@"coverType"] integerValue] == 0) {
            ...
            vc.dict = self.ML_newpersonArray[indexPath.row];
