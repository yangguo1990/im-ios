//
//  ML_MineFocusSSViewController.m
//  miliao
//
...
@interface ML_MineFocusSSViewController ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation ML_MineFocusSSViewController
...
-(void)giveML_focusApi{
    kSelf;
    MLGetUserFriendsApi *api = [[MLGetUserFriendsApi alloc]initWithtoUserId:@"" token:[ML_AppUserInfoManager sharedManager].currentLoginUserData.token type:@"1" page:@(self.ML_Page) limit:@"20" extra:[self jsonStringForDictionary]];
    [api networkWithCompletionSuccess:^(MLNetworkResponse *response) {
  
        NSMutableArray *arr = response.data[@"friends"];
        NSLog(@"%@",self.data);
