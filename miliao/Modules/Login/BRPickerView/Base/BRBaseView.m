...
- (UIView *)NmaskView {
    if (!_NmaskView) {
        _NmaskView = [[UIView alloc]initWithFrame:self.keyView.bounds];
        _NmaskView.backgroundColor = self.pickerStyle.maskColor;
        // 设置子视图的大小随着父视图变化
        _NmaskView.autoresizingMask = UIViewAutoresizingFlexibleBottomMargin | UIViewAutoresizingFlexibleRightMargin | UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        _NmaskView.userInteractionEnabled = YES;
        UITapGestureRecognizer *myTap = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(didTapMaskView:)];
        [_NmaskView addGestureRecognizer:myTap];
    }
    return _NmaskView;
}
...
- (void)addPickerToView:(UIView *)view {
    if (view) {
        ...
        // 弹出动画
        if (!self.pickerStyle.hiddenMaskView) {
            self.NmaskView.alpha = 0;
        }
        [UIView animateWithDuration:0.3 animations:^{
            if (!self.pickerStyle.hiddenMaskView) {
                self.NmaskView.alpha = 1;
            }
