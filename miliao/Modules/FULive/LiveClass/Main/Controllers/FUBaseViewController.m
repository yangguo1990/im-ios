...
//     }];
    ...
    _noTrackLabel.text = Localized(@"未检测到人脸", nil);
    ...
    if((ML_NavViewHeight > 64)){
        _photoBtn.center = CGPointMake(self.view.frame.size.width/2, self.view.frame.size.height - 85 - ML_TabbarHeight);
    }else{
...
- (void)video:(NSString *)videoPath didFinishSavingWithError:(NSError *)error contextInfo:(void *)contextInfo{
    if(error != NULL){
//        [MBProgressHUD showText:Localized(@"保存视频失败", nil)];
        [SVProgressHUD showErrorWithStatus:Localized(@"保存视频失败", nil)];
        
    }else{
//        [MBProgressHUD showText:Localized(@"视频已保存到相册", nil)];
        [SVProgressHUD showSuccessWithStatus:Localized(@"视频已保存到相册", nil)];
    }
}
...
-(void)displayPromptText{
    BOOL isHaveFace = [[FUManager shareManager] isTracking];
    dispatch_async(dispatch_get_main_queue(), ^{
        self.noTrackLabel.text = Localized(@"未检测到人脸", nil);
