...
static FUNewManager *shareManager = NULL;

@implementation FUNewManager

+ (FUNewManager *)shareManager
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        shareManager = [[FUNewManager alloc] init];
    });
    ...
    dispatch_async(_asyncLoadQueue, ^{
        [[FURenderer shareRenderer] setUpCurrentContext];
           if (param.type == FUDataTypeBeautify) {
               ...
               int handle = [[FUNewManager shareManager] getHandleAboutType:FUNamaHandleTypeBeauty];
...
//    FUBeautyParam *selectParameter = [UserLocalDataTool getBeautyFilterParam];
    FUBeautyParam *selectParameter = nil;
    if (selectParameter) {
        int selectIndex = 2;
...
-(void)setupSkinData{
    ...
    //从本地获取保存的参数
//    NSArray *skinArray = [UserLocalDataTool getBeautySkinArrays];
    NSArray *skinArray = @[];
    if (skinArray.count) {
...
-(void)setupShapData{
   NSArray *prams = @[@"cheek_thinning",@"cheek_v",@"cheek_narrow",@"cheek_small",@"eye_enlarging",@"intensity_chin",@"intensity_forehead",@"intensity_nose",@"intensity_mouth",@"intensity_canthus",@"intensity_eye_space",@"intensity_eye_rotate",@"intensity_long_nose",@"intensity_philtrum",@"intensity_smile"];
    NSDictionary *titelDic = @{@"cheek_thinning":@"瘦脸",@"cheek_v":Localized(@"v脸", nil),@"cheek_narrow":Localized(@"窄脸", nil),@"cheek_small":Localized(@"小脸", nil),@"eye_enlarging":@"大眼",@"intensity_chin":@"下巴",
...
//    NSArray *shapeArray = [UserLocalDataTool getBeautyShapeArrays];
    NSArray *shapeArray = @[];
    if (shapeArray.count) {
