...
-(void)setupSkinData{
    ...
    NSDictionary *titelDic = @{@"blur_level":Localized(@"精细磨皮", nil),@"color_level":@"美白",@"red_level":@"红润",@"sharpen":Localized(@"锐化", nil),@"remove_pouch_strength":Localized(@"去黑眼圈", nil),@"remove_nasolabial_folds_strength":Localized(@"去法令纹", nil),@"eye_bright":@"亮眼",@"tooth_whiten":@"美牙"};
    NSDictionary *defaultValueDic = @{@"blur_level":@(0.7),@"color_level":@(0.7),@"red_level":@(0.3),@"sharpen":@(0.2),@"remove_pouch_strength":@(0),@"remove_nasolabial_folds_strength":@(0),@"eye_bright":@(0),@"tooth_whiten":@(0)};
...
-(void)setupShapData{
   NSArray *prams = @[@"cheek_thinning",@"cheek_v",@"cheek_narrow",@"cheek_small",@"intensity_cheekbones",@"intensity_lower_jaw",@"eye_enlarging",@"intensity_chin",@"intensity_forehead",@"intensity_nose",@"intensity_mouth",@"intensity_canthus",@"intensity_eye_space",@"intensity_eye_rotate",@"intensity_long_nose",@"intensity_philtrum",@"intensity_smile"];
    NSDictionary *titelDic = @{@"cheek_thinning":@"瘦脸",@"cheek_v":Localized(@"v脸", nil),@"cheek_narrow":Localized(@"窄脸", nil),@"cheek_small":Localized(@"小脸", nil),@"intensity_cheekbones":Localized(@"瘦颧骨", nil),@"intensity_lower_jaw":Localized(@"瘦下颌骨", nil),@"eye_enlarging":@"大眼",@"intensity_chin":@"下巴",
                               @"intensity_forehead":@"额头",@"intensity_nose":@"瘦鼻",@"intensity_mouth":@"嘴型",@"intensity_canthus":@"开眼角",@"intensity_eye_space":Localized(@"眼距", nil),@"intensity_eye_rotate":Localized(@"眼睛角度", nil),@"intensity_long_nose":Localized(@"长鼻", nil),@"intensity_philtrum":Localized(@"缩人中", nil),@"intensity_smile":Localized(@"微笑嘴角", nil)
    };
...
- (void)destoryItems{
    dispatch_async(_asyncLoadQueue, ^{
         NSLog(@"44444strat Nama destroy all items ~");
...
- (void)destoryItemAboutType:(FUNamaHandleType)type;
{
    dispatch_async(_asyncLoadQueue, ^{
        /**后销毁老道具句柄*/
        if (items[type] != 0) {
            NSLog(@"55555faceunity: destroy item");
...
- (void)loadGesture
{
    dispatch_async(_asyncLoadQueue, ^{
        if (items[FUNamaHandleTypeGesture] != 0) {
            NSLog(@"6666666faceunity: destroy gesture");
...
- (void)loadFilter{
    ...
    //从本地获取保存的参数
    NSArray *skinArray = [FYAppManager.shareInstanse fy_getBeautySkinArrays];
    if (skinArray.count) {
        [_skinParams removeAllObjects];
        [_skinParams addObjectsFromArray:skinArray];
    }

    
    NSArray *shapeArray = [FYAppManager.shareInstanse fy_getBeautyShapeArrays];
     if (shapeArray.count) {
         [_shapeParams removeAllObjects];
         [_shapeParams addObjectsFromArray:shapeArray];
     }
    
    FUBeautyParam *selectParameter = [FYAppManager.shareInstanse fy_getBeautyFilterParam];
    if (selectParameter) {
        int selectIndex = 2;
        for (int i = 0; i < _filters.count; i++) {
            FUBeautyParam *model = _filters[i];
            if ([model.mParam isEqualToString:selectParameter.mParam]) {
                model.mValue = selectParameter.mValue;
                selectIndex = i;
                break;
            }
        }
        self.seletedFliter = _filters[selectIndex];
    }else
    {
        self.seletedFliter = _filters[2];
    }
...
- (void)setBeautyParameters{
    ...
    for (FUBeautyParam *modle in _shapeParams){
        ...
        NSLog(@"设置----参数（%@）-----值（%lf)",modle.mParam,modle.mValue);
     }
...
- (void)setBeautyDefaultParameters:(FUBeautyModuleType)type{
    if((type & FUBeautyModuleTypeSkin) == FUBeautyModuleTypeSkin){
        ...
        [FYAppManager.shareInstanse fy_saveBeautySkinArrays:_skinParams];
    }
    ...
    if((type & FUBeautyModuleTypeShape) == FUBeautyModuleTypeShape){
        ...
        [FYAppManager.shareInstanse fy_saveBeautyShapeArrays:_shapeParams];
    }
}

#pragma mark 这里美肤参数做本地保存
...
- (void)loadItem:(NSString *)itemName completion:(void (^)(BOOL finished))completion{
    dispatch_async(_asyncLoadQueue, ^{
        ...
        if (destoryItem != 0)
        {
            NSLog(@"77777faceunity: destroy item");
...
- (void)loadBundleWithName:(NSString *)name aboutType:(FUNamaHandleType)type{
    dispatch_async(_asyncLoadQueue, ^{
        if (items[type] != 0) {
            NSLog(@"88888faceunity: destroy item");
            [FURenderer destroyItem:items[type]];
            items[type] = 0;
        }
...
- (void)loadMakeupBundleWithName:(NSString *)name{
    dispatch_async(_asyncLoadQueue, ^{
        if (items[FUNamaHandleTypeMakeup] != 0) {
            NSLog(@"9999999faceunity: destroy item");
...
- (void)setEnableGesture:(BOOL)enableGesture
{
    ...
    if (_enableGesture) {
        ...
        if (items[FUNamaHandleTypeGesture] != 0) {
            ...
            NSLog(@"000000faceunity: destroy gesture");
