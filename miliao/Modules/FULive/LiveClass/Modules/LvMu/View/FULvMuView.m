...
-(void)setupSubView{
    ...
    [_restBtn setTitle:Localized(@"恢复", nil) forState:UIControlStateNormal];
    ...
    __weak typeof(self)weakSelf  = self ;
    _segmentBarView = [[FUSegmentBarView alloc] initWithFrame:CGRectMake(0, 200,[UIScreen mainScreen].bounds.size.width, 49) titleArray:@[Localized(@"抠像",nil),Localized(@"背景",nil)] selBlock:^(int index) {
    ...
    param0.mTitle = Localized(@"取消", nil);
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == _mItemCollectionView) {
        ...
        if (indexPath.row < self.dataArray.count){
            ...
            cell.titleLabel.text = Localized(modle.mTitle,nil);
...
-(void)resetBtnClick:(UIButton *)btn{
    ...
    UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:nil message:Localized(@"是否将所有参数恢复到默认值",nil) preferredStyle:UIAlertControllerStyleAlert];
