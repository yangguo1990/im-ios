...
/* 多人脸展示蒙版 */
@property (strong, nonatomic)UIView *NmaskView;
/* 合成海报是否保存 */
...
-(void)initializationView{
    ...
    [_OKBtn setTitle:Localized(@"知道了", nil) forState:UIControlStateNormal];
        ...
        if ([FURenderer isTracking] == 1) {
            ...
            if (![self isGoodFace:0]) {
                ...
                if (_PushFrom == FUEditImagePushFromPhoto) {
                    self.mTextLabel.text = Localized(@"人脸偏转角度过大，请重新拍摄",nil);
                }else{
                    self.mTextLabel.text = Localized(@"人脸偏转角度过大，请重新选择",nil);
                }
...
- (void)selectFace:(UITapGestureRecognizer *)tap {
    CGPoint point = [tap locationInView:self.NmaskView] ;
    for (NSValue *value in _array) {
        CGRect rect = [value CGRectValue];
        if (CGRectContainsPoint(rect,point)) {
            [_NmaskView removeFromSuperview];
...
-(void)addMakeView:(int)faceNum photoWidth:(int)width photoHeight:(int)height iamgeAspectRect:(CGRect)iamgeAspectRect{
    //创建一个View
    _NmaskView = [[UIView alloc] initWithFrame:[UIScreen mainScreen].bounds];
    [_NmaskView setBackgroundColor:[UIColor colorWithWhite:0 alpha:0.6]];
    [self.view addSubview:_NmaskView];
    
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectFace:)];
    [self.NmaskView addGestureRecognizer:tap];
    
    UIButton *tipBtn = [[UIButton alloc] initWithFrame:CGRectMake((_NmaskView.frame.size.width - 200)/2, 0, 200, 40)];
    [tipBtn setTitle:Localized(@"检测到多人，请选择一人进行换脸", nil)  forState:UIControlStateNormal];
    ...
    [self.NmaskView addSubview:tipBtn];
    
    UIBezierPath *path = [UIBezierPath bezierPathWithRect:_NmaskView.bounds];
    for (int i = 0; i < faceNum; i ++) {
        ...
        if (CGRectGetMaxY(rect) > tipBtn.frame.origin.y) {
            tipBtn.frame = CGRectMake((_NmaskView.frame.size.width - 200)/2, CGRectGetMaxY(rect) + 20, 200, 40);
        }
    ...
    //添加图层蒙板
    _NmaskView.layer.mask = shapeLayer;
...
-(void)setMPhotoImage:(UIImage *)mPhotoImage{
    ...
    if (_PushFrom == FUEditImagePushFromAlbum) {
        _mTextLabel.text = Localized(@"未检测出人脸，请重新上传",nil);
    }else{
        _mTextLabel.text = Localized(@"未检测出人脸，请重新拍摄",nil); 
    }
...
- (IBAction)loadAction:(id)sender {
    ...
    if (_mImageView.image) {
        if (_isSave) {
//            kToast(Localized(@"图片已保存到相册", nil));
//             [SVProgressHUD showSuccessWithStatus:Localized(@"图片已保存到相册", nil)];
            return;
        }
...
{
    if(error != NULL){
//        [MBProgressHUD showText:Localized(@"保存图片失败", nil)];
        [SVProgressHUD showErrorWithStatus:Localized(@"保存图片失败", nil)];
    }else{
        _isSave = YES;
//        [MBProgressHUD showText:Localized(@"图片已保存到相册", nil)];
        [SVProgressHUD showSuccessWithStatus:Localized(@"图片已保存到相册", nil)];
    }
}
