...
//    self.imageView.backgroundColor = [UIColor grayColor];
    ...
    NSMutableAttributedString *string = [[NSMutableAttributedString alloc] initWithString:Localized(@"一键变表情", nil) attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:17],NSForegroundColorAttributeName: [UIColor colorWithRed:255/255.0 green:255/255.0 blue:255/255.0 alpha:1.0]}];
    ...
    [_tipView.OKBtn  setTitle:Localized(@"知道了", nil) forState:UIControlStateNormal];
    ...
    if ([FURenderer isTracking] > 0) {
        if (![[FUManager shareManager] isGoodFace:0]) {
            if (_pushFrom == FUGanEditImagePushFromPhoto) {
                _tipView.mTextLabel.text = Localized(@"人脸偏转角度过大，请重新拍摄",nil);
            }else{
                _tipView.mTextLabel.text = Localized(@"人脸偏转角度过大，请重新选择",nil);
            }
            _tipView.hidden = NO;
        ...
        if ([[FUManager shareManager] isExaggeration:0]) {
            _tipView.mTextLabel.text = Localized(@"人脸表情夸张，请正脸拍摄",nil);
            _tipView.hidden = NO;
            CFRelease(photoDataFromImageDataProvider);
            return;
        }
        ...
        if (_pushFrom == FUGanEditImagePushFromPhoto) {
            _tipView.mTextLabel.text = Localized(@"未检测出人脸，请重新拍摄",nil);
        }else{
            _tipView.mTextLabel.text = Localized(@"未检测出人脸，请重新选择",nil);
        }
        _tipView.hidden = NO;
        CFRelease(photoDataFromImageDataProvider);
        return;
...
-(void)backBtnClick:(UIButton *)btn{
    ...
    UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:nil message:Localized(@"返回后当前操作将不会保存哦",nil) preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *cancleAction = [UIAlertAction actionWithTitle:Localized(@"取消", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
        
    }];
    [cancleAction setValue:[UIColor colorWithRed:44/255.f green:46/255.f blue:48/255.f alpha:1] forKey:@"titleTextColor"];
    
    UIAlertAction *certainAction = [UIAlertAction actionWithTitle:Localized(@"确定", nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
-(void)syntheticExpressionPhoto:(UIImage *)image expressionIndex:(int)index{
    [SVProgressHUD showWithStatus:Localized(@"正在处理图片", nil)];
...
//    [manager POST:@"http://192.168.0.86:5001/index_upload" parameters:dict headers:nil constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
