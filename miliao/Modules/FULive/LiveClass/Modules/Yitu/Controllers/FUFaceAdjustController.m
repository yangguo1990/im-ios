...
-(void)setupView{
    ...
    [_miniAdjustBtn setTitle:Localized(@"精细调整",nil) forState:UIControlStateNormal];
...
-(void)showAlert{
    UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:nil message:Localized(@"你还没有添加五官哦",nil) preferredStyle:UIAlertControllerStyleAlert];
    
    UIAlertAction *cancleAction = [UIAlertAction actionWithTitle:Localized(@"不添加",nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
    ...
    UIAlertAction *certainAction = [UIAlertAction actionWithTitle:Localized(@"去添加",nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
...
-(void)backBtnClick:(UIButton *)btn{
    ...
    UIAlertAction *certainAction = [UIAlertAction actionWithTitle:Localized(@"继续",nil) style:UIAlertActionStyleDefault handler:^(UIAlertAction * _Nonnull action) {
       [self.navigationController popViewControllerAnimated:YES];
    }];
    [certainAction setValue:[UIColor colorWithRed:134/255.f green:157/255.f blue:255/255.f alpha:1] forKey:@"titleTextColor"];
...
-(void)saveBtnClick:(UIButton *)btn{
    ...
    [SVProgressHUD showSuccessWithStatus:Localized(@"道具保存成功",nil)];
...
-(void)yituAddItemView:(FUYituItemModel *)model{
    if ([self currentNumType:model.type] == 3) {
        [self showMessage:Localized(@"不能添加更多了哦~",nil)];
        return;
    }
