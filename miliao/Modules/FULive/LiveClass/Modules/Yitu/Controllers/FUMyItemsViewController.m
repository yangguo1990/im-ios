...
//    self.noItemLabel.hidden = [FUManager shareManager].myStickers.count != 0;
    self.noItemLabel.text = Localized(@"暂无制作道具哦~",nil);
    self.mTitleLabel.text = Localized(@"删除道具",nil);
    [self.selectAllBtn setTitle:Localized(@"全选",nil) forState:UIControlStateNormal];
     [self.deleteBtn setTitle:Localized(@"删除", nil) forState:UIControlStateNormal];
...
- (IBAction)selectAllItems:(UIButton *)sender {
    ...
    if (_dataArray.count == self.selectedIndex.count) {
        ...
        self.selectAllBtn.titleLabel.text =Localized(@"全选",nil);
        [self.selectAllBtn setTitle:Localized(@"全选",nil) forState:UIControlStateNormal ];
    }else {
        ...
        self.selectAllBtn.titleLabel.text = Localized(@"取消", nil);
        [self.selectAllBtn setTitle:Localized(@"取消", nil) forState:UIControlStateNormal ];
    }
...
- (IBAction)deleteAction:(UIButton *)sender {
    ...
    UIAlertController *alertCon = [UIAlertController alertControllerWithTitle:nil message:Localized(@"确定删除所选中的道具？",nil) preferredStyle:UIAlertControllerStyleAlert];
...
- (void)deleteBtnRefreshTitle {
    ...
    if (self.selectedIndex.count == 0) {
        ...
        self.deleteBtn.titleLabel.text = Localized(@"删除", nil);
        [self.deleteBtn setTitle:Localized(@"删除", nil) forState:UIControlStateNormal];
    }else {
        self.bottomView.backgroundColor = [UIColor colorWithRed:94/255.0 green:199/255.0 blue:254/255.0 alpha:1.0];
        self.deleteBtn.titleLabel.text = [NSString stringWithFormat:Localized(@"删除(%ld)",nil), (unsigned long)self.selectedIndex.count];
        [self.deleteBtn setTitle:[NSString stringWithFormat:Localized(@"删除(%ld)",nil), (unsigned long)self.selectedIndex.count] forState:UIControlStateNormal];
    }
}
