...
@interface ML_MsgListVC ()<JXCategoryViewDelegate/*,BMKGeneralDelegate,BMKLocationAuthDelegate,BMKLocationManagerDelegate*/>
...
- (void)showMoreBtn:(NSNotification *)nt
{
    BOOL notShow = [nt.object boolValue];
    
    self.moreBtn.hidden = notShow;
}

- (void)viewDidLoad {
    [super viewDidLoad];
    self.view.backgroundColor = UIColor.whiteColor;
    ADD_NOTIFICATION(@"showMoreBtn", self, @selector(showMoreBtn:), nil);
...
//        make.top.mas_equalTo(self.bgimg.mas_bottom).mas_offset(-21);
    ...
    self.moreBtn = searchImg;
    
    self.categoryView = [[JXCategoryDotView alloc]init];

    
    self.categoryView.titles = @[Localized(@"消息", nil), Localized(@"通话记录", nil)];
...
- (void)setupChildVces{
    ...
    ML_MsgZiListVC *cityvc = [[ML_MsgZiListVC alloc]init];
    [self addChildViewController:cityvc];
    
    ML_TonghuaListVC *recommendvc = [[ML_TonghuaListVC alloc] init];
...
-(void)searchClick {
    ...
    POST_NOTIFICATION(@"ClickedMoreBtn", nil, nil);
