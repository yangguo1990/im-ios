...
#import "ML_AppPopupParam.h"
#import "ML_AppPopupExtension.h"

//如果弹框view不设tag值,那么弹框的tag值将设为此值
#define kPopupDefaultTag (201912)

/* 动画数据源代理,询问动画的各种配置
 * 弹框本身自动实现,若将代理设为其他类则其他类需要实现以下方法
 */
@protocol WorkplaceApppPopupDataSource<NSObject>
...
/* 弹框的渐隐动画设置
 * popup:弹框本类
 * state:有两个参数WorkplaceApppPopupStateShow,WorkplaceApppPopupStateHide分别设置弹框展示时的参数和参考消失时的参数
 * animationWithAlphaForState:弹出和消失时分别是否需要渐隐动画
 * fromAlphaForState:弹出和消失时起始alpha值
 * toAlphaForState:弹出和消失时结束alpha值
 */
- (BOOL    )tf_popupView:(UIView *)popup animationWithAlphaForState:(WorkplaceApppPopupState)state;//默认返回YES
- (CGFloat )tf_popupView:(UIView *)popup fromAlphaForState:(WorkplaceApppPopupState)state;//默认show=0,hide=1
- (CGFloat )tf_popupView:(UIView *)popup toAlphaForState:(WorkplaceApppPopupState)state;//默认show=1,hide=0
/* 弹框的尺寸【位移】动画设置
 * popup:弹框本类
 * state:有两个参数WorkplaceApppPopupStateShow,WorkplaceApppPopupStateHide分别设置弹框展示时的参数和参考消失时的参数
 ...
 */
- (BOOL    )tf_popupView:(UIView *)popup animationWithFrameForState:(WorkplaceApppPopupState)state;//默认返回YES
//默认show&hide大小为弹框原来尺寸,位置为弹框区域的终点位置
- (CGRect  )tf_popupView:(UIView *)popup fromFrameForState:(WorkplaceApppPopupState)state;
- (CGRect  )tf_popupView:(UIView *)popup toFrameForState:(WorkplaceApppPopupState)state;//同上
 ...
 */
@optional;
- (NSTimeInterval)tf_popupView:(UIView *)popup animationDurationForState:(WorkplaceApppPopupState)state;//默认0.3
- (NSTimeInterval)tf_popupView:(UIView *)popup animationDelayForState:(WorkplaceApppPopupState)state;//默认0.0
 ...
 */
@protocol WorkplaceApppPopupBackgroundDelegate<NSObject>
/* 弹出背景视图设置
 * popup:弹框本类
 ...
 */
@protocol WorkplaceApppPopupDelegate<NSObject>
@optional;
/* 监听获取参数前后
 * popup:弹框本类
...
@interface UIView (WorkplaceApppPopup)<WorkplaceApppPopupDataSource,WorkplaceApppPopupDelegate,WorkplaceApppPopupBackgroundDelegate>

@property(nonatomic,  weak)UIView *inView;//弹框的容器视图

@property(nonatomic,  copy)NSString *identifier;//标示符，用于在弹出池里查找弹出的view

@property(nonatomic,strong)ML_AppPopupExtension *extension;

@property(nonatomic,assign)id<WorkplaceApppPopupDelegate>popupDelegate;//动画调用过程代理
@property(nonatomic,assign)id<WorkplaceApppPopupDataSource>popupDataSource;//动画所有配置代理
@property(nonatomic,assign)id<WorkplaceApppPopupBackgroundDelegate>backgroundDelegate;//动画背景视图设置代理

@property(nonatomic,strong)ML_AppPopupParam *popupParam;//默认动画参数
 ...
 * popupParam 弹出的更多参数设置，具体参照ML_AppPopupParam类 */
...
-(void)tf_showSlide:(UIView *)inView direction:(PopupDirection)direction popupParam:(ML_AppPopupParam *)popupParam;
 ...
 * inView 容器视图
 * popupParam 弹出的更多参数设置，具体参照ML_AppPopupParam类
 * delegate 动画代理，默认为弹框本类，具体请参照WorkplaceApppPopupDelegate类 */
...
@interface CAAnimation (WorkplaceApppPopup)<CAAnimationDelegate>
