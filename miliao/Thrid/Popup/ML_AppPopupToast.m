...
#import "ML_AppPopupToast.h"
#import "UIView+TFPopup.h"

@interface ML_AppPopupToast ()<WorkplaceApppPopupDelegate>

@property(nonatomic,assign)CGRect inViewFrame;
@property(nonatomic,  copy)NSString *msg;
@property(nonatomic,  copy)ML_AppPopupToastBlock cusBlock;

@end

@implementation ML_AppPopupToast

-(void)dealloc{
#if DEBUG
    NSLog(@"****** ML_AppPopupToast(已释放) msg:%@ ******",self.msg);
#endif
}

+(void)tf_show:(UIView *)inView msg:(NSString *)msg animationType:(TFAnimationType)animationType{
    [ML_AppPopupToast tf_show:inView
                      msg:msg
                   offset:CGPointZero
         dissmissDuration:0
...
dissmissDuration:(NSTimeInterval)duration
 animationType:(TFAnimationType)animationType
   customBlock:(ML_AppPopupToastBlock)customBlock{
    ...
    param.offset = offset;
    if (duration == 0) {
        param.autoDissmissDuration = [ML_AppPopupToast toastDuration:msg];
    }else{
        param.autoDissmissDuration = duration;
    }
    
    ML_AppPopupToast *toast = [[ML_AppPopupToast alloc]initWithFrame:CGRectZero];
