...
@interface WorkplaceApppPopupPrivateExtension : NSObject
...
@end
@implementation WorkplaceApppPopupPrivateExtension
@end

@implementation UIView (WorkplaceApppPopup)
...
-(void)tf_showNormal:(UIView *)inView popupParam:(ML_AppPopupParam *)popupParam{
    
    self.inView = inView;
    self.popupParam = popupParam;
    if (self.popupParam == nil)self.popupParam = [[ML_AppPopupParam alloc]init];
    [self setDefault];
...
-(void)tf_showScale:(UIView *)inView offset:(CGPoint)offset{
    [self tf_showScale:inView offset:offset popupParam:[ML_AppPopupParam new]];
}

-(void)tf_showScale:(UIView *)inView offset:(CGPoint)offset popupParam:(ML_AppPopupParam *)popupParam{
...
-(void)tf_showSlide:(UIView *)inView direction:(PopupDirection)direction{
    [self tf_showSlide:inView direction:direction popupParam:[ML_AppPopupParam new]];
}

-(void)tf_showSlide:(UIView *)inView
          direction:(PopupDirection)direction
         popupParam:(ML_AppPopupParam *)popupParam{
...
-(void)tf_showcustom:(UIView *)inView popupParam:(ML_AppPopupParam *)popupParam{
    
#ifdef DEBUG
    NSAssert(inView != nil, @"(WorkplaceApppPopup)you must setting a inView.");
    NSAssert([inView isKindOfClass:[UIView class]], @"(WorkplaceApppPopup)inView must be a view.");
#endif
    if (self.tag == 0) {self.tag = kPopupDefaultTag;}
    self.inView = inView;
    self.popupParam = popupParam;
    if (self.popupParam == nil)self.popupParam = [[ML_AppPopupParam alloc]init];
...
-(void)tf_reload{
    ...
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationWithAlphaForState:)]) {
        BOOL animation = [self.popupDataSource tf_popupView:self animationWithAlphaForState:WorkplaceApppPopupStateShow];
    ...
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:fromAlphaForState:)]) {
        self.extension.showFromAlpha = [self.popupDataSource tf_popupView:self fromAlphaForState:WorkplaceApppPopupStateShow];
    ...
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:toAlphaForState:)]) {
        self.extension.showToAlpha = [self.popupDataSource tf_popupView:self toAlphaForState:WorkplaceApppPopupStateShow];
    }
    
    /* show-frame */
    //show-frame是否动画
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationWithFrameForState:)]) {
        BOOL animation = [self.popupDataSource tf_popupView:self animationWithFrameForState:WorkplaceApppPopupStateShow];
    ...
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:fromFrameForState:)]) {
        self.extension.showFromFrame = [self.popupDataSource tf_popupView:self fromFrameForState:WorkplaceApppPopupStateShow];
        self.frame = self.extension.showFromFrame;
    }
    //show-frame结束时的frame
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:toFrameForState:)]) {
        self.extension.showToFrame = [self.popupDataSource tf_popupView:self toFrameForState:WorkplaceApppPopupStateShow];
    }
    
    /* hide-alpha */
    //hide-alpha是否动画
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationWithAlphaForState:)]) {
        BOOL animation = [self.popupDataSource tf_popupView:self animationWithAlphaForState:WorkplaceApppPopupStateHide];
        self.extension.disuseHideAlphaAnimation = !animation;
    }
    //hide-alpha开始时的alpha
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:fromAlphaForState:)]) {
        self.extension.hideFromAlpha = [self.popupDataSource tf_popupView:self fromAlphaForState:WorkplaceApppPopupStateHide];
    }
    //hide-alpha结束时的alpha
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:toAlphaForState:)]) {
        self.extension.hideToAlpha = [self.popupDataSource tf_popupView:self toAlphaForState:WorkplaceApppPopupStateHide];
    }
    
    /* hide-frame */
    //hide-frame是否动画
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationWithFrameForState:)]) {
        BOOL animation = [self.popupDataSource tf_popupView:self animationWithFrameForState:WorkplaceApppPopupStateHide];
        self.extension.disuseHideFrameAnimation = !animation;
    }
    //hide-frame开始时的frame
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:fromFrameForState:)]) {
        self.extension.hideFromFrame = [self.popupDataSource tf_popupView:self fromFrameForState:WorkplaceApppPopupStateHide];
    }
    //hide-frame结束时的frame
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:toFrameForState:)]) {
        self.extension.hideToFrame = [self.popupDataSource tf_popupView:self toFrameForState:WorkplaceApppPopupStateHide];
    }
    ...
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationDurationForState:)]) {
        self.extension.showAnimationDuration = [self.popupDataSource tf_popupView:self animationDurationForState:WorkplaceApppPopupStateShow];
    }
    //show动画延迟
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationDelayForState:)]) {
        self.extension.showAnimationDelay = [self.popupDataSource tf_popupView:self animationDelayForState:WorkplaceApppPopupStateShow];
    }
    //show动画选项
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationOptionsForState:)]) {
        self.extension.showAnimationOptions = [self.popupDataSource tf_popupView:self animationOptionsForState:WorkplaceApppPopupStateShow];
    }
    ...
    //hide动画时间
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationDurationForState:)]) {
        self.extension.hideAnimationDuration = [self.popupDataSource tf_popupView:self animationDurationForState:WorkplaceApppPopupStateHide];
    }
    //hide动画延迟
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationDelayForState:)]) {
        self.extension.hideAnimationDelay = [self.popupDataSource tf_popupView:self animationDelayForState:WorkplaceApppPopupStateHide];
    }
    //hide动画选项
    if ([self.popupDataSource respondsToSelector:@selector(tf_popupView:animationOptionsForState:)]) {
        self.extension.hideAnimationOptions = [self.popupDataSource tf_popupView:self animationOptionsForState:WorkplaceApppPopupStateHide];
    }
...
-(void)tf_show{
    ...
    WorkplaceApppPopupPrivateExtension *ext = getRunCache(self);
    ...
    //将弹窗放进弹窗池
    [WorkplaceApppPopupPool addToPool:self];
...
+(UIView *)tf_findPopup:(NSString *)identifier{
    return [WorkplaceApppPopupPool findPopup:identifier];
}

//获取所有已弹出的弹窗
+(NSArray <UIView *>*)tf_getAllPopup{
    return [WorkplaceApppPopupPool allPopup];
}
...
#pragma mark -- WorkplaceApppPopupDataSource
...
- (BOOL    )tf_popupView:(UIView *)popup animationWithAlphaForState:(WorkplaceApppPopupState)state{
    switch (state) {
        case WorkplaceApppPopupStateShow:{
            return !self.popupParam.disuseShowPopupAlphaAnimation;
        }break;
        case WorkplaceApppPopupStateHide:{
...
- (CGFloat )tf_popupView:(UIView *)popup fromAlphaForState:(WorkplaceApppPopupState)state{
    switch (state) {
        case WorkplaceApppPopupStateShow:{
            return 0;
        }break;
...
- (CGFloat )tf_popupView:(UIView *)popup toAlphaForState:(WorkplaceApppPopupState)state{
    switch (state) {
        case WorkplaceApppPopupStateShow:{
            return 1;
        }break;
...
- (BOOL    )tf_popupView:(UIView *)popup animationWithFrameForState:(WorkplaceApppPopupState)state{
    switch (state) {
        case WorkplaceApppPopupStateShow:{
...
- (CGRect  )tf_popupView:(UIView *)popup fromFrameForState:(WorkplaceApppPopupState)state{
    switch (state) {
        case WorkplaceApppPopupStateShow:{
...
- (CGRect  )tf_popupView:(UIView *)popup toFrameForState:(WorkplaceApppPopupState)state{
    switch (state) {
        case WorkplaceApppPopupStateShow:{
            if (CGRectEqualToRect(self.popupParam.popTargetFrame, CGRectZero) == NO) {
                return self.popupParam.popTargetFrame;
...
- (NSTimeInterval)tf_popupView:(UIView *)popup animationDurationForState:(WorkplaceApppPopupState)state{
    if (self.popupParam.duration > 0) {
        return self.popupParam.duration;
    }
    return kDefaultAnimationDuration;
}
- (NSTimeInterval)tf_popupView:(UIView *)popup animationDelayForState:(WorkplaceApppPopupState)state{
    if (state == WorkplaceApppPopupStateShow) {
        return MAX(self.popupParam.showAnimationDelay, 0.0);
    }else if(state == WorkplaceApppPopupStateHide){
        return MAX(self.popupParam.hideAnimationDelay, 0.0);
    }
    return 0;
}
- (UIViewAnimationOptions)tf_popupView:(UIView *)popup animationOptionsForState:(WorkplaceApppPopupState)state{
...
#pragma mark -- WorkplaceApppPopupBackgroundDelegate
...
#pragma mark -- WorkplaceApppPopupDelegate
...
-(void)tf_remove{
    ...
    [self.extension.backgroundViewArray removeAllObjects];
    [self.extension.backgroundViewFrameArray removeAllObjects];
    [WorkplaceApppPopupPool refreshPool];
}
...
static inline WorkplaceApppPopupPrivateExtension *getRunCache(NSObject *obj){
    if (obj == nil) {
        return nil;
    }
    ...
    WorkplaceApppPopupPrivateExtension *ext = [_tfPopupRunCache objectForKey:addr];
    if (ext == nil) {
        ext = [[WorkplaceApppPopupPrivateExtension alloc]init];
...
static inline CGRect normalOriginFrame(ML_AppPopupParam *param){
...
static inline CGRect normalTargetFrame(ML_AppPopupParam *param){
    CGRect ar = param.popupAreaRect;
    CGSize s = param.popupSize;
    CGPoint st = param.offset;
    CGFloat x = (ar.size.width - s.width) * 0.5 + st.x;
...
static inline CGRect slideOriginFrame(ML_AppPopupParam *param,PopupDirection direction){
    CGRect ar = param.popupAreaRect;
    CGSize s = param.popupSize;
    CGPoint st = param.offset;
...
static inline CGRect slideTargetFrame(ML_AppPopupParam *param,PopupDirection direction){
    CGRect ar = param.popupAreaRect;
    CGSize s = param.popupSize;
    CGPoint st = param.offset;
    CGFloat x = (ar.size.width - s.width) * 0.5;
...
-(ML_AppPopupExtension *)extension{
    id value = objc_getAssociatedObject(self, @selector(extension));
    if (value) {
        return value;
    }else{
        self.extension = [[ML_AppPopupExtension alloc]init];
        value = self.extension;
    }
    return value;;
}
-(void)setExtension:(ML_AppPopupExtension *)extension{
...
@implementation CAAnimation (WorkplaceApppPopup)
