...
//    _suspensionViewWH = 50.0 * scale;
    ...
    CGFloat suspensionViewH = 134 * screenH / screenW;
    _suspensionFrame = CGRectMake(x, y, self.suspensionViewWH, suspensionViewH);
...
- (void)setNavCtr:(UINavigationController *)navCtr {
    [_navCtr setNavigationBarHidden:YES];
...
- (void)setupSuspensionViewWithTargetVC:(UIViewController<JPSuspensionEntranceProtocol> *)targetVC suspensionXY:(CGPoint)suspensionXY {
    [targetVC.navigationController setNavigationBarHidden:YES];
    CGFloat screenW = [UIScreen mainScreen].bounds.size.width;
    CGFloat screenH = [UIScreen mainScreen].bounds.size.height;
    CGFloat h = 134 * screenH / screenW;
    _suspensionFrame = CGRectMake(suspensionXY.x, suspensionXY.y, _suspensionViewWH, h);
    [self fixSuspensionFrame];
...
//    [self.navCtr setNavigationBarHidden:YES];
...
//    [targetVC.navigationController setNavigationBarHidden:YES];
    ...
    [self.navCtr pushViewController:targetVC animated:NO];
}
