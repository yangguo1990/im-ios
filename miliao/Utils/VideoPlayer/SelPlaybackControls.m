...
- (void)_retryButtonShow:(BOOL)show
{
    ...
    if (show) {
        ...
        self.failcontentlabel.text = @"视频播放出错，请稍后再试！";
        self.failcontentlabel.hidden = NO;
    }else
    {
        self.retryButton.hidden = YES;
        self.failcontentlabel.hidden = YES;
    }
}
...
- (void)setupUI
{
    ...
    //添加提示性文字----
    [self addSubview:self.failcontentlabel];
...
- (void)makeConstraints
{
    ...
    [_failcontentlabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.center.equalTo(self);
    }];
    
    [_fullScreenButton mas_makeConstraints:^(MASConstraintMaker *make) {
        //make.right.bottom.equalTo(_bottomControlsBar);
        make.right.mas_equalTo(self.bottomControlsBar).mas_offset(-25);
        make.bottom.mas_equalTo(self.bottomControlsBar);
        make.size.mas_equalTo(CGSizeMake(30, 30));
    }];
    
    [_playTimeLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(_bottomControlsBar).offset(25);
...
/** 加载播放失败提示性文字 */
-(UILabel *)failcontentlabel
{
    if (!_failcontentlabel) {
        _failcontentlabel = [[UILabel alloc]init];
        _failcontentlabel.font = [UIFont systemFontOfSize:14];
        _failcontentlabel.adjustsFontSizeToFitWidth = YES;
        _failcontentlabel.textAlignment = NSTextAlignmentCenter;
        _failcontentlabel.textColor = [UIColor whiteColor];
    }
    return _failcontentlabel;
}
