...
- (void)fetchImageObject:(PHAsset *)asset complection:(void (^)(UIImage *,NSURL *,NSInteger,NSURL *))complection
{
...
//                complection(nil,nil,type,urlAsset.URL);
...
//        }];
        ...
        [[PHImageManager defaultManager] requestImageForAsset:phAsset targetSize:imageSize contentMode:PHImageContentModeAspectFit options:option resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
            ...
            if (downloadFinined) {
                ...
                complection(image,imageurl,type,nil);
            }
        }];
    }
...
//                if ([asset isKindOfClass:[AVURLAsset class]]) {
//                    AVURLAsset* urlAsset = (AVURLAsset*)asset;
//                    UIImage *image = [self getImage:urlAsset.URL];
//                    NSNumber *size;
//                    [urlAsset.URL getResourceValue:&size forKey:NSURLFileSizeKey error:nil];
//                    NSLog(@"size is %f",[size floatValue]/(1024.0*1024.0));
//                    complection(image,urlAsset.URL);
//             }
//            }];
//        }else{
//            PHImageRequestOptions *option = [[PHImageRequestOptions alloc] init];
