...
- (void)startRecording {
    if (TARGET_IPHONE_SIMULATOR) {
        NSLog(@"录制视频不支持模拟器");
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"录制视频不支持模拟器" delegate:nil cancelButtonTitle:Localized(@"确定", nil) otherButtonTitles:nil];
        [alertView show];
        return;
    }
    @synchronized(self) {
...
- (BOOL)addDefaultCameraInputToCaptureSession:(AVCaptureSession *)captureSession {
   ...
   // AVCaptureDeviceInput *cameraDeviceInput = [[AVCaptureDeviceInput alloc] initWithDevice:[AVCaptureDevice defaultDeviceWithMediaType:AVMediaTypeVideo] error:&error];
    AVCaptureDevice * cameraFront =  [self cameraWithPosition:AVCaptureDevicePositionFront];
   
    
    AVCaptureDeviceInput *cameraDeviceInput  = [AVCaptureDeviceInput deviceInputWithDevice:cameraFront error:nil];
