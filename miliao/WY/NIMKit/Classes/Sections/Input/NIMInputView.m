...
- (CGSize)sizeThatFits:(CGSize)size
{
    ...
    switch (self.status)
    {
        ...
        default:
        {
            ...
            CGFloat W = ML_ScreenWidth / 4;
            int i = 0;
            for (UIView *view in vc.view.subviews) {
                if (view.tag >= 1000 && view.tag < 1004) {
                    NSInteger tag = view.tag - 1000;
                    if ([NIMKitKeyboardInfo instance].isVisiable) {
                        view.frame = CGRectMake(W * tag, ML_ScreenHeight - [NIMKitKeyboardInfo instance].keyboardHeight - 50, W, 50);
                        
                        if ([ML_AppUtil isCensor]) {
                            NSLog(@"asdfasdf=asdf=dsf=asf===%f", CGRectGetMaxY(self.inputView.frame));
                            if (i == 2) {
                                view.frame = CGRectMake(0, view.y, ML_ScreenWidth / 2, 50);
                            } else if (i == 3){
                                view.frame = CGRectMake(ML_ScreenWidth / 2, view.y, ML_ScreenWidth / 2, 50);
                            }
                        }
                        
                        
                    } else {
                        view.frame = CGRectMake(W * tag, ML_ScreenHeight - ML_TabbarHeight, W, 50);
                        if ([ML_AppUtil isCensor]) {
                            NSLog(@"asdfasdf=asdf=dsf=asf===%f", CGRectGetMaxY(self.inputView.frame));
                            if (i == 2) {
                                view.frame = CGRectMake(0, view.y, ML_ScreenWidth / 2, 50);
...
- (void)setup
{
    ...
    //设置placeholder
//    NSString *placeholder = [NIMKit sharedKit].config.placeholder; // 发消息的配置取值
    NSString *placeholder = Localized(@"发消息", nil); // 发消息的配置取值
    ...
    [_toolBar.recordButton setTitle:Localized(@"按住说话", nil).nim_localized forState:UIControlStateNormal];
    [_toolBar.recordButton setHidden:YES];
    
    //    int count = [ML_AppUtil isCensor]?3:4;
    //    CGFloat W = ML_ScreenWidth / count;
    //    for (int i = 0; i < count; i++) {
    //        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(W * i, 50, W, 50)];
    //        [btn addTarget:self action:@selector(sessionInputViewbottBtnClick:) forControlEvents:UIControlEventTouchUpInside];
    //        btn.tag = 10+i;
    //        [self.sessionInputView addSubview:btn];
    //        if (i == 0) {
...
- (void)checkEmoticonContainer
{
    ...
    CGFloat W = ML_ScreenWidth / 4;
    int i = 0;
    for (UIView *view in [UIViewController topShowViewController].view.subviews) {
        if (view.tag >= 1000 && view.tag < 1004) {

            view.frame = CGRectMake(W * i, ML_ScreenHeight - ML_TabbarHeight, W, 50);
            if ([ML_AppUtil isCensor]) {
                NSLog(@"asdfasdf=asdf=dsf=asf===%f", CGRectGetMaxY(self.inputView.frame));
                if (i == 2) {
                    view.frame = CGRectMake(0, view.y, ML_ScreenWidth / 2, 50);
              ...
              if (granted) {
                  ...
                  dispatch_async(dispatch_get_main_queue(), ^{
                      [[[UIAlertView alloc] initWithTitle:nil
                                                 ...
                                                 delegate:nil
                                        cancelButtonTitle:Localized(@"确定", nil).nim_localized
...
- (IBAction)onTouchRecordBtnDown:(id)sender {
    ...
    if ([self.isFaStr length]) {
        
        if ([self.isFaStr isEqualToString:@"0"]) {
            kplaceToast(@"没有发语音权限");
        } else {
            kplaceToast(self.isFaStr);
        }
...
- (IBAction)onTouchRecordBtnUpInside:(id)sender {
    // finish Recording
    self.recordPhase = AudioRecordPhaseEnd;
    NSLog(@"onTouchRecordBtnUpInside====");
}
- (IBAction)onTouchRecordBtnUpOutside:(id)sender {
    // cancel Recording
    self.recordPhase = AudioRecordPhaseEnd;
    NSLog(@"onTouchRecordBtnUpOutside====");
}
