...
- (void)setup
{
    ...
    _imageView.layer.cornerRadius = self.nim_width / 2;
...
- (void)setAvatarBySession:(NIMSession *)session
{
    ...
    NSLog(@"asdf--asdf---%@", info.avatarUrlString);
}
...
- (void)didSetImageUrl:(NSURL *)url placeholderImage:(UIImage *)placeholderImage options:(SDWebImageOptions)options {
    ...
    __weak typeof(self) weakSelf = self;
//    UIImage *fixedPlaceholderImage  = [self imageAddCorner:placeholderImage radius:_cornerRadius andSize:self.bounds.size];
