...
@implementation NIMMessageCell // 私聊cell组件
...
- (void)makeComponents
{
    ...
    //headerView
    _headImageView = [[NIMAvatarImageView alloc] initWithFrame:CGRectMake(0, 0, 36, 36)];
    ...
    _bubblesBackgroundView.layer.cornerRadius = 4;
...
- (void)layoutBubbleView
{
    ...
    {
        CGFloat right = self.model.shouldShowAvatar? CGRectGetMinX(self.headImageView.frame)  - protraitRightToBubble : self.nim_width;
        left = right - CGRectGetWidth(self.bubbleView.bounds) - 3;
    } else {
        if (![self needShowSelectButton]) {
            left = contentInsets.left + 3;
        } else {
            left = contentInsets.left + _selectButton.nim_right + protraitRightToBubble + 3;
        }
    }
...
- (CGRect)avatarViewRect
{
    ...
    return CGRectMake(selfProtraitOriginX, self.cellPaddingToAvatar.y+3,protraitImageWidth,protraitImageHeight);
}
