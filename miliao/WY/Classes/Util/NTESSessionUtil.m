...
+(NSString*)weekdayStr:(NSInteger)dayOfWeek
{
    ...
    daysOfWeekDict = @{@(1):Localized(@"星期日", nil),
                       @(2):Localized(@"星期一", nil),
                       @(3):Localized(@"星期二", nil),
                       @(4):Localized(@"星期三", nil),
                       @(5):Localized(@"星期四", nil),
...
+ (NSString *)getPeriodOfTime:(NSInteger)time withMinute:(NSInteger)minute
{
    ...
    {
        showPeriodOfTime = Localized(@"凌晨", nil);
    }
    else if (totalMin > 5 * 60 && totalMin < 12 * 60)
    {
        showPeriodOfTime = Localized(@"上午", nil);
    }
    else if (totalMin >= 12 * 60 && totalMin <= 18 * 60)
    {
        showPeriodOfTime = Localized(@"下午", nil);
    }
    else if ((totalMin > 18 * 60 && totalMin <= (23 * 60 + 59)) || totalMin == 0)
    {
        showPeriodOfTime = Localized(@"晚上", nil);
    }
...
+ (NSString *)tipOnMessageRevokedTwo:(id)message
{
    ...
    if ([info isKindOfClass:[NIMSubscribeOnlineInfo class]] && info.senderClientTypes.count)
    {
        ...
        switch (event.value) {
            ...
            default:
            {
                ...
                state = [NSString stringWithFormat:@"%@ %@", clientName, Localized(@"在线", nil).ntes_localized];
                break;
            }
        }
    }
...
{
    NSString *clientName = [self resolveOnlineClientName:client];
    NSString *state = [NSString stringWithFormat:@"%@ %@",clientName,Localized(@"在线", nil).ntes_localized];
    ...
    if (dict) {
        ...
        switch (onlineState) {
            case NTESOnlineStateNormal:
            {
                ...
                {
                    //桌面端不显示网络状态，只显示端
                    return [NSString stringWithFormat:@"%@ %@",clientName,Localized(@"在线", nil).ntes_localized];
                }
                else
                {
                    ...
                    if (detail)
                    {
                        return [NSString stringWithFormat:@"%@ - %@ %@",clientName,netState, Localized(@"在线", nil).ntes_localized];
                    }
                    else
                    {
                        return [NSString stringWithFormat:@"%@ %@",netState,Localized(@"在线", nil).ntes_localized];
                    }
                }
            }
