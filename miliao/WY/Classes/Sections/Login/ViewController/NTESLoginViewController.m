...
- (void)configNav{
    ...
    [loginBtn setTitle:Localized(@"完成", nil).ntes_localized forState:UIControlStateNormal];
...
- (void)prepareShowLog:(UILongPressGestureRecognizer *)gesuture{
    if (gesuture.state == UIGestureRecognizerStateBegan) {
        ...
        UIActionSheet *actionSheet = [[UIActionSheet alloc] initWithTitle:nil delegate:nil cancelButtonTitle:Localized(@"取消", nil).ntes_localized destructiveButtonTitle:nil otherButtonTitles:@"查看SDK日志".ntes_localized,@"查看Demo日志".ntes_localized, nil];
