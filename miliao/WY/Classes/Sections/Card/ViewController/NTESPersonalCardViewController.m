...
- (void)setUpNav{
    ...
    if ([self.userId isEqualToString:[[NIMSDK sharedSDK].loginManager currentAccount]]) {
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:Localized(@"编辑", nil) style:UIBarButtonItemStyleDone target:self action:@selector(onActionEditMyInfo:)];
                        ...
                        @{
                            ...
                            RowContent :@[
                                           @{
                                            ...
                                            @{
                                                Title        : Localized(@"生日", nil).ntes_localized,
                         ...
                         @{
                          ...
                          @{
                            ...
                            RowContent :@[
                                           @{
                                                Title        : Localized(@"黑名单", nil).ntes_localized,
                                                CellClass    : @"NTESSettingSwitcherCell",
...
- (void)chat{
    ...
    ML_SessionViewController *vc = [[ML_SessionViewController alloc] initWithSession:session];
...
- (void)addFriend{
    ...
    NSString *successText = request.operation == NIMUserOperationAdd ? @"添加成功".ntes_localized : @"请求成功";
    NSString *failedText =  request.operation == NIMUserOperationAdd ? @"添加失败".ntes_localized : Localized(@"请求失败", nil);
...
- (void)deleteFriend{
    __weak typeof(self) wself = self;
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"删除好友".ntes_localized message:@"删除好友后，将同时解除双方的好友关系".ntes_localized delegate:nil cancelButtonTitle:Localized(@"取消", nil).ntes_localized otherButtonTitles:Localized(@"确定", nil).ntes_localized, nil];
...
- (void)onLongPressNavbar:(UIGestureRecognizer *)gesture{
    if (gesture.state == UIGestureRecognizerStateBegan) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"个人信息".ntes_localized message:title delegate:nil cancelButtonTitle:Localized(@"确定", nil).ntes_localized otherButtonTitles:nil, nil];
        [alert show];
    }
}
