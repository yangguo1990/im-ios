...
- (IBAction)onJionTeamBtnClick:(id)sender {
    ...
    if(self.joinTeam.joinMode == NIMTeamJoinModeNoAuth) {
        ...
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"" message:@"输入验证信息".ntes_localized delegate:nil cancelButtonTitle:Localized(@"确定", nil).ntes_localized otherButtonTitles:nil, nil];
...
- (void)handleApplyToTeam:(NSError *)error status:(NIMTeamApplyStatus)status {
    ...
    NSLog(@"Jion team status: %zd", status);
}
