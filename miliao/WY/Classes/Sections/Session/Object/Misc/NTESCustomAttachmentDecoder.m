    ...
    if (data) {
        ...
        if ([dict isKindOfClass:[NSDictionary class]])
        {
            ...
            switch (type) {
                ...
                default:
                {
                    NSDictionary * info = dict[LVInfo];
                    NSString * cmd =info[LVCMD];
                    if([cmd isEqualToString:LVGiftCMD]){
                        attachment = [ML_GiftAttachment initWithDic:dict];
                    }
                }
                    break;
            }
...
- (BOOL)checkAttachment:(id<NIMCustomAttachment>)attachment{
    ...
    }else if([attachment isKindOfClass:[ML_GiftAttachment class]]){
        check = YES;
    }
    return check;
}
