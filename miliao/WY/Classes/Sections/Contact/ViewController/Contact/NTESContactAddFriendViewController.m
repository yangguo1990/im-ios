...
- (void)addFriend:(NSString *)userId{
    ...
    [[NIMSDK sharedSDK].userManager fetchUserInfos:@[userId] completion:^(NSArray *users, NSError *error) {
        ...
        if (users.count) {
            ...
            if (wself) {
                UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"该用户不存在".ntes_localized message:@"请检查你输入的帐号是否正确".ntes_localized delegate:nil cancelButtonTitle:Localized(@"确定", nil).ntes_localized otherButtonTitles:nil, nil];
                [alert show];
            }
        }
    }];
