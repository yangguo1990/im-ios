...
- (instancetype)init
{
    if (self = [super init])
    {
        _types =  @[
                   ...
                   NSStringFromClass([ML_GiftAttachment class]),
                   ];
...
//    }
    if([self isTipTextMessage:model.message]){
        return YES;
    }
    return [super disableRetryButton:model];
}

-(BOOL) isTipTextMessage:(NIMMessage *) message
{
   if (message.messageType == NIMMessageTypeCustom) {
       NIMCustomObject *object = message.messageObject;
       if ([object.attachment isKindOfClass:[ML_CustomTipsAttachment class]]) {
           return YES;
       }
   }
   return NO;
